...
const Members: React.FC = () => {
  ...
  const fetchAllData = async (userId: string) => {
    ...
    try {
      // Fetch all members
      const allMembersResponse = await fetch('http://localhost:8888/api/members/all');
      if (!allMembersResponse.ok) throw new Error('Failed to fetch all members');
      ...
      const followersResponse = await fetch('http://localhost:8888/api/members/follows/followers', {
        method: 'POST',
      ...
      const followingResponse = await fetch('http://localhost:8888/api/members/follows/following', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: userId }),
      });
      if (!followingResponse.ok) throw new Error('Failed to fetch who user is following');
  ...
  const renderMembers = () => {
    ...
    if (membersToDisplay.length === 0) {
      return <p className="text-gray-400 text-center">No members found in this list.</p>;
    }

    return (
      <>
        <h2 className="text-3xl font-bold text-gray-100 text-center mb-8">{title}</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {membersToDisplay.map((member) => {
            const isCurrentUser = member.user_id === currentUserId;
            const isFollowing = following.some(f => f.user_id === member.user_id);
            
            return (
              <div 
                key={member.user_id} 
                className={`flex items-start p-6 rounded-2xl shadow-lg transition-transform hover:scale-105 hover:shadow-2xl 
                  ${isCurrentUser ? 'bg-indigo-700 text-white border-2 border-indigo-400' : 'bg-slate-800 text-gray-100'}`
                }
              >
                <img 
                  src={member.pfp_url} 
                  ...
                  onError={(e) => {
                    ...
                    e.currentTarget.src="https://placehold.co/100x100/1F2937/FFFFFF?text=PFP";
                  }} 
                />
                <div className="flex-grow">
                  <h3 className="text-xl font-bold mb-1">{member.username}</h3>
                  <p className="text-sm text-gray-300 mb-2">{member.bio}</p>
                  <p className="text-xs text-gray-400 italic mb-4">{member.recentActivity}</p>
  ...
  if (isLoading) {
    return <div className="text-center text-lg font-medium text-gray-400 mt-20">Loading members...</div>;
  }

  if (error) {
    return <div className="text-center text-lg font-medium text-red-500 mt-20">Error: {error}</div>;
  }
  ...
  return (
    <div className="font-sans p-8 max-w-7xl mx-auto bg-slate-900 text-gray-100 min-h-screen">
      <nav className="flex justify-center gap-4 mb-8">
        <button 
          className={`px-6 py-3 rounded-full font-semibold transition-all ${currentView === 'all' ? 'bg-teal-600 text-white shadow-md' : 'bg-slate-800 text-gray-400 hover:bg-slate-700'}`} 
          onClick={() => setCurrentView('all')}
        >
          All Members
        </button>
        <button 
          className={`px-6 py-3 rounded-full font-semibold transition-all ${currentView === 'followers' ? 'bg-teal-600 text-white shadow-md' : 'bg-slate-800 text-gray-400 hover:bg-slate-700'}`} 
          onClick={() => setCurrentView('followers')}
        >
          Followers ({followers.length})
        </button>
        <button 
          className={`px-6 py-3 rounded-full font-semibold transition-all ${currentView === 'following' ? 'bg-teal-600 text-white shadow-md' : 'bg-slate-800 text-gray-400 hover:bg-slate-700'}`} 
